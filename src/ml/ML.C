...
float ML::EvalAllMetric(int *coords, Metrics *metrics) {
...
float ML::EvalSpecificMetric(int *coords) {
...
void ML::GetNthPermutation(int n, int size, int *params) {
  for (int i = 0; i < size; i++) 
    params[i] = i + 1;
  for (int i = 0; i < n && next_permutation(params, params + size); i++);
}

void ML::DumpParamToFile(int *params) {
...
void ML::MapParamsToSearchSpace(int *params, vector<int> *mappedParams) {
...
void ML::ForcePointToRange(int *point) {
  for (int i = 0; i < GetTotalParams(); i++) {
...
int ML::GetIndexFromCoord(int dims, int *coord) {
  int index = 0;
...
void ML::SetValue(int *coord, float val) {
...
void ML::AddValue(int *coord, float val) {
  int index = GetIndexFromCoord(GetTotalParams(), coord);

  mlData item;
  item.params = (int *) malloc(sizeof(int) * GetTotalParams());
  for (int i = 0; i < GetTotalParams(); i++) {
    item.params[i] = coord[i];
  }
  item.index = index;
  item.val = val;
  item.visited = false;
...
void ML::ReadData(string filename) {
